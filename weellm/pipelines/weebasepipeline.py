        ...
        if hasattr(pipeline, "vae") and hasattr(pipeline.vae, "decode"):
            ...
            def defrag_vae_decode(self_obj, *args, **kwargs):
                ...
                if hasattr(pipeline, "vae_streamer"):
                    streamer_decode = pipeline.vae_streamer.decode
                    if getattr(streamer_decode, "__func__", None) is defrag_vae_decode:
                        # vae_streamer has no real decode override of its own --
                        # BaseVAEStreamer.__getattr__ proxies unknown attributes
                        # straight to self.model, which is this same pipeline.vae,
                        # so "vae_streamer.decode" resolves back to this very
                        # patched method and would recurse forever. Use the
                        # original implementation instead in that case.
                        res = original_vae_decode_vram(*args, **kwargs)
                    else:
                        res = streamer_decode(*args, **kwargs)
